...
export class DictionaryService {
  translates: Array<ITranslate> = JSON.parse(
  ...
  addTranslate(
    ...
    if (
      this.translates.some(
        (t: ITranslate) =>
          t.word1 === translate.word1 &&
          t.lang1 === translate.lang1 &&
          t.lang2 === translate.lang2
      )
    ) {
      return;
    }
  ...
  getTranslates(): Observable<Array<ITranslate>> {
    return this.translates$.pipe(
      map((list) =>
        list
          .filter(
            (t) =>
              t.lang1 === this.settingsService.fromLang &&
  ...
  getGoWords(): Array<IGoWord> {
    const translatesToGo = this.translates.filter(
      (t) =>
        t.lang1 === this.settingsService.fromLang &&
        t.lang2 === this.settingsService.toLang
    );
    if (translatesToGo.length < this.settingsService.goWordsCount) {
      return translatesToGo.map((t) => {
        return {
          word1: t.word1,
          word2: t.word2,
        };
